  ...
  let insert_seg t (seq, fin, data) =
          ...
          | None ->
            ...
            *)
            if Sequence.less_equal seq e.seq then
              (* case (a) *)
              let seq_e = Sequence.addi seq (String.length data) in
              ...
              *)
              if Sequence.less_equal seq_e e.seq then
                if Sequence.equal seq_e e.seq then
                  let e = { seq ; fin = fin || e.fin ; data = Rope.prepend data e.data } in
                  Some (e, Sequence.addi seq (Rope.length e.data)), e :: acc
                else
                  let e' = { seq ; fin ; data = Rope.of_string data } in
                  Some (e', Sequence.addi seq (String.length data)), e :: e' :: acc
              else
                ...
                if Sequence.greater_equal seq_e e_seq_e then
                  ...
                  in
                  let e = { seq ; fin = fin || e.fin ; data = Rope.prepend data post } in
                  Some (e, Sequence.addi seq (Rope.length e.data)), e :: acc
            else
              let e_seq_e = Sequence.addi e.seq (Rope.length e.data) in
              if Sequence.less_equal seq e_seq_e then
                ...
                if Sequence.equal seq e_seq_e then
                  let e = { e with fin = fin || e.fin ; data = Rope.append e.data data } in
                  Some (e, Sequence.addi e_seq_e (String.length data)), e :: acc
                else
                  ...
                  let post =
                    if Sequence.greater e_seq_e seq_e then
                      ...
                      Rope.empty
                  in
                  let e = { e with fin = fin || e.fin ; data = Rope.concat (Rope.append pre data) post } in
                  Some (e, end_), e :: acc
              else
                (None, e :: acc))
        (None, []) t
    ...
    let segq =
      if inserted = None then
        { seq ; fin ; data = Rope.of_string data } :: segq
      else
        segq
    in
    List.rev segq
