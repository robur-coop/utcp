...
83 98 00 00 02 04 05 b4 03 03 06 00|}
  ...
  in
  Alcotest.(check int "real1" 0x8399 Segment.(checksum_bytes ~src ~dst data))
...
89 8e 28 be 00 00 00 00|}
  ...
  in
  Alcotest.(check int "real2" 0x42f3 Segment.(checksum_bytes ~src ~dst data))
...
89 8e 28 be 00 00 00 00|}
  in
  let data = poor_shift ((String.make 25 '\000') ^ data) 25 in
  let data = Bytes.of_string data in
  let src = Ipaddr.(V4 (V4.of_string_exn "10.0.42.1"))
  and dst = Ipaddr.(V4 (V4.of_string_exn "10.0.42.2"))
  in
  Alcotest.(check int "real3" 0x42f3 Segment.(checksum_bytes ~src ~dst data))
...
89 8e 28 be 00 00 00 00  02 02 02|}
  ...
  and dst = Ipaddr.(V4 (V4.of_string_exn "10.0.42.2"))
  in
  Alcotest.(check int "real4" 16110 Segment.(checksum_bytes ~src ~dst data))
...
89 8e 28 be 00 00 00 00  02 02|}
  ...
  and dst = Ipaddr.(V4 (V4.of_string_exn "10.0.42.2"))
  in
  Alcotest.(check int "real5" 16623 Segment.(checksum_bytes ~src ~dst data))
